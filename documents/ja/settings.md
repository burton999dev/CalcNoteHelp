...
アクションバーを表示する|OFFにすると画面上部のアクションバーが非表示になります。編集領域を大きくしたい場合に有効な設定です。
フルスクリーンモード|`CalcNote`をフルスクリーンモードで表示します。
長いファイル名を省略する|アクションバーに表示されるファイル名が長い場合、`...`で省略して表示します。OFFの場合はファイル名をスクロールして全体を表示します。

## キーパッドの設定
|項目|説明|
|:-----------|:------------|
キー配列|キーパッドの配置を`電卓配列`、`ケータイ配列`で選択できます。
高さをソフトウェアキーボードに合わせる|ONの場合、キーパッドの高さがソフトウェアキーボードの高さにマッチするように調整されます。**この設定を有効にするにはアプリの再起動が必要です。**
...
|項目|説明|
|:-----------|:------------|
表示形式|結果領域の表示フォーマットを設定します。小数点をカンマで表現したりする国もあるので、国際化対応のための設定です。
...
|:-----------|:------------|
消費税率(%)|消費税率を設定します。
消費税計算時の小数点桁数|消費税計算時の小数点桁数です。日本では小数点以下は端数処理されるため0です。
消費税の端数処理|消費税の端数処理方法を設定します。日本では切り捨てが一般的ですが、法的には切り上げでも問題なく、お店によって異なる可能性があります。
...
<br>
[HOME](index.md)　[基本操作](how2use.md)　[演算子と関数](operator_and_function.md)　[使用例](usecases.md)  
