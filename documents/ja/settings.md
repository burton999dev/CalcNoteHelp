...
## テーマ
テーマを適用すると外観の設定(アクションバーやボタンの色)を簡単に切り替えることができます。サポートしているテーマは `デフォルト` `ピンク` `パープル` `ブルー` `ティール` `グリーン` `ライトグリーン` `ライム` `オレンジ` `ブラウン`の全10種類です。  
テーマはテンプレートのようなもので、テーマを適用した後に気になる部分を微調整することも可能です。  
現在、ユーザーの皆様からもテーマを募集しているので今後も新しいテーマを追加予定です。
