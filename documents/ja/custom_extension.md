...
get(url)|引数に指定した`URL`に`HTTP GET`してレスポンスを文字列で返します。通信は同期的に行われるので、アプリが一瞬フリーズしたようになります。レスポンスは1時間キャッシュされます。あくまで試験的な機能で利用は推奨しません。
...
### 制限事項
1. Javascriptはオープンソースの[Rhino v1.7.7.1](https://github.com/mozilla/rhino)によって実行されます。よって使用できるJavascriptの機能はRhinoがサポートしているものに限られます。
2. 関数は毎回コンパイルされてから実行されます。端末のスペックにもよりますが、複雑な関数やファイル内でユーザー定義関数の呼び出しを複数回行なうとパフォーマンスに影響する可能性があります。
3. 特に`get`関数を使ったHTTPリクエストは同期的に実行されるのでパフォーマンスに影響します。
