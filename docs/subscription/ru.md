---
title: CalcNote Pro
---

# CalcNote Pro

### 1. Реклама полностью исчезает
Все рекламные блоки в приложении скрываются, и пользоваться им становится комфортнее.

- Убирается полноэкранная реклама при запуске (раз в 6 часов), поэтому можно сразу начать расчёты.
- Исчезают баннеры в верхней части экранов, освобождая больше места для содержимого.
- Нативные объявления в боковом меню и в настройках тоже скрываются, что ускоряет доступ к нужным функциям.

### 2. Свободное управление историей изменений файлов
Работайте с файлами спокойнее.
<sub><a href="https://www.youtube.com/watch?v=M9TqYfH1bqo" target="_blank"><img src="https://raw.githubusercontent.com/burton999dev/CalcNoteHelp/refs/heads/master/images/all/yt_icon_red_digital.png" width="32" height="32" style="vertical-align: text-bottom; margin-left: 6px;"></a></sub>

- Каждый запуск сохранения автоматически записывается в историю. (**История сохраняется и в бесплатном плане, поэтому после перехода на Pro вы сразу получите доступ к прошлым версиям.**)
- В Файловом менеджере можно открыть список версий и вернуть любой нужный сохранённый вариант.
- Легко восстановить файлы, удалённые по ошибке.

### 3. Доступен Парящий виджет
Удобный виджет для расчётов и заметок, пока другие приложения остаются открытыми.
<sub><a href="https://www.youtube.com/watch?v=paxUzu6x7sE" target="_blank"><img src="https://raw.githubusercontent.com/burton999dev/CalcNoteHelp/refs/heads/master/images/all/yt_icon_red_digital.png" width="32" height="32" style="vertical-align: text-bottom; margin-left: 6px;"></a></sub>

- Отображается полупрозрачно поверх экрана, позволяя проверять результаты или делать заметки без прерывания работы.
- Запускается в любой момент одним нажатием из строки состояния.
- Например, можно записать цену из браузера, не переключаясь между приложениями.

### 4. Управляйте файлами с помощью папок
Упорядочивайте растущие заметки с расчётами по назначению.
<sub><a href="https://www.youtube.com/watch?v=gi9GtEi-RB4" target="_blank"><img src="https://raw.githubusercontent.com/burton999dev/CalcNoteHelp/refs/heads/master/images/all/yt_icon_red_digital.png" width="32" height="32" style="vertical-align: text-bottom; margin-left: 6px;"></a></sub>

- Создавайте папки по проектам или темам и храните связанные файлы вместе.
- Даже при большом количестве файлов структура по папкам помогает быстро найти нужную заметку.

### 5. Усильте безопасность приложения с помощью код-пароля
Настройте блокировку код-паролем при запуске приложения, чтобы повысить безопасность.

- Снизьте риск случайного редактирования файлов, когда вы временно даете устройство другому человеку.
- Если каждый раз вводить код-пароль неудобно, можно использовать биометрическую аутентификацию, поддерживаемую устройством.

### 6. Неограниченное число пользовательских списков
Вы можете создать сколько угодно **пользовательских списков**, чтобы упорядочить часто используемые значения и единицы.
<sub><a href="https://www.youtube.com/watch?v=2UZjLNSWFs0" target="_blank"><img src="https://raw.githubusercontent.com/burton999dev/CalcNoteHelp/refs/heads/master/images/all/yt_icon_red_digital.png" width="32" height="32" style="vertical-align: text-bottom; margin-left: 6px;"></a></sub>

- В бесплатном плане доступен один пользовательский список; Pro снимает ограничение.
- Используйте разные списки под разные задачи и значительно повысите эффективность.
