---
title: CalcNote Pro
---

# CalcNote Pro

### 1. Реклама повністю зникає
Усі рекламні оголошення в застосунку приховуються, що робить використання комфортнішим.

- Прибирається повноекранна реклама під час запуску (раз на 6 годин), тож одразу після відкриття можна рахувати.
- Зникають банери у верхній частині екранів, звільняючи більше місця для вмісту.
- Приховуються і нативні оголошення в бічному меню та в налаштуваннях, тож ви швидше дістанетесь потрібних функцій.

### 2. Вільне керування історією змін файлів
Працюйте з файлами спокійніше.
<sub><a href="https://www.youtube.com/watch?v=M9TqYfH1bqo" target="_blank"><img src="https://raw.githubusercontent.com/burton999dev/CalcNoteHelp/refs/heads/master/images/all/yt_icon_red_digital.png" width="32" height="32" style="vertical-align: text-bottom; margin-left: 6px;"></a></sub>

- Кожне збереження автоматично фіксується в історії. (**Історія зберігається й у безплатному плані, тому після переходу на Pro одразу маєте доступ до попередніх версій.**)
- У Файловому менеджері можна переглянути список історії та відновити будь-яку точку збереження.
- Легко відновлюються файли, видалені помилково.

### 3. Доступний Плаваючий віджет
Зручний віджет для обчислень і нотаток, поки інші програми відкриті.
<sub><a href="https://www.youtube.com/watch?v=paxUzu6x7sE" target="_blank"><img src="https://raw.githubusercontent.com/burton999dev/CalcNoteHelp/refs/heads/master/images/all/yt_icon_red_digital.png" width="32" height="32" style="vertical-align: text-bottom; margin-left: 6px;"></a></sub>

- Відображається напівпрозоро поверх екрана, тож можна перевіряти результати чи робити нотатки без перерви.
- Запускається будь-коли одним натисканням зі смуги стану.
- Наприклад, можна занотувати ціну з браузера, не перемикаючи застосунки.

### 4. Керуйте файлами за допомогою папок
Охайно впорядковуйте зростаючі нотатки з розрахунками за призначенням.
<sub><a href="https://www.youtube.com/watch?v=gi9GtEi-RB4" target="_blank"><img src="https://raw.githubusercontent.com/burton999dev/CalcNoteHelp/refs/heads/master/images/all/yt_icon_red_digital.png" width="32" height="32" style="vertical-align: text-bottom; margin-left: 6px;"></a></sub>

- Створюйте папки за проєктами або темами та зберігайте пов’язані файли разом.
- Навіть за великої кількості файлів структура папок допоможе швидко знайти потрібну нотатку.

### 5. Посильте безпеку застосунку за допомогою код-пароля
Налаштуйте блокування код-паролем під час запуску застосунку, щоб посилити безпеку.

- Зменште ризик випадкового редагування файлів, коли ви тимчасово даєте пристрій іншій людині.
- Якщо щоразу вводити код-пароль незручно, можна використовувати біометричну автентифікацію, яку підтримує пристрій.

### 6. Необмежена кількість користувацьких списків
Можете створювати скільки завгодно **користувацьких списків**, щоб впорядковувати часто вживані значення та одиниці.
<sub><a href="https://www.youtube.com/watch?v=2UZjLNSWFs0" target="_blank"><img src="https://raw.githubusercontent.com/burton999dev/CalcNoteHelp/refs/heads/master/images/all/yt_icon_red_digital.png" width="32" height="32" style="vertical-align: text-bottom; margin-left: 6px;"></a></sub>

- У безплатному плані дозволений один користувацький список; у Pro обмеження немає.
- Використовуйте різні списки під різні задачі, щоб суттєво підвищити ефективність.
